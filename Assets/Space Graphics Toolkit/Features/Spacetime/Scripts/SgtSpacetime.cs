...
namespace SpaceGraphicsToolkit
{
	...
	public class SgtSpacetime_Editor : SgtEditor
	{
		protected override void OnInspector()
		{
			...
			BeginError(Any(tgts, t => t.Size == Vector2.zero));
			...
			EndError();
			BeginError(Any(tgts, t => t.Tile <= 0));
			...
			BeginError(Any(tgts, t => t.AmbientBrightness < 0.0f));
			...
			BeginError(Any(tgts, t => t.DisplacementBrightness < 0.0f));
			...
			BeginError(Any(tgts, t => t.HighlightPower < 0.0f));
			...
			BeginIndent();
				...
				if (Any(tgts, t => t.Displacement == SgtSpacetime.DisplacementType.Pinch))
				{
				...
				if (Any(tgts, t => t.Displacement == SgtSpacetime.DisplacementType.Offset))
				{
			...
			if (Any(tgts, t => t.Mesh == null && t.GetComponent<SgtSpacetimeMesh>() == null))
			{
				Separator();

				if (Button("Add Mesh") == true)
				{
					Each(tgts, t => SgtHelper.GetOrAddComponent<SgtSpacetimeMesh>(t.gameObject));
				}
			}

			if (dirtyMaterial == true)
			{
				Each(tgts, t => t.DirtyMaterial(), true);
			}
		}

		[UnityEditor.MenuItem(SgtHelper.GameObjectMenuPrefix + "Spacetime", false, 10)]
