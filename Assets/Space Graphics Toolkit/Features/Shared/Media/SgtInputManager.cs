	...
	public class SgtInputManager : MonoBehaviour
	{
			...
			public float GetValue(float delta)
			{
				...
				if (FingerCount > 0 && fingers.Count == FingerCount)
				{
					switch (FingerGesture)
					{
						...
						case AxisGesture.HorizontalPull: value += GetAveragePullScaled(fingers).x * FingerSensitivity * delta; break;
						case AxisGesture.VerticalPull: value += GetAveragePullScaled(fingers).y * FingerSensitivity * delta; break;
					}
				}
		...
		public struct Trigger
		{
			...
			public bool WentDown(Finger finger)
			{
				...
				if (UseKey != KeyCode.None && finger.Index == HOVER_FINGER_INDEX && SgtInputManager.WentDown(UseKey) == true)
				{
					return true;
				}
			...
			public bool IsDown(Finger finger)
			{
				...
				if (UseKey != KeyCode.None && finger.Index == HOVER_FINGER_INDEX && SgtInputManager.IsDown(UseKey) == true)
				{
					return true;
				}
			...
			public bool WentUp(Finger finger, bool useAnyFinger = false)
			{
				...
				if (UseKey != KeyCode.None && finger.Index == HOVER_FINGER_INDEX && SgtInputManager.WentUp(UseKey) == true)
				{
					return true;
				}
		...
		public static void EnsureThisComponentExists()
		{
			...
			{
				new GameObject(typeof(SgtInputManager).Name).AddComponent<SgtInputManager>();
			}
		}
